...
class PreferenceUtil {
    ...
    /// 设置用户信息
    static func setWxInfo(_ data: WxInfo) {
        UserDefaults.standard.set(data.toJSONString()!, forKey: KEY_WX_INFO)
    }
    
    /// 获取用户信息
    static func getWxInfo() -> WxInfo? {
        let data = UserDefaults.standard.string(forKey: KEY_WX_INFO)
        if let json = data, !json.isEmpty {
            return JsonUtil.jsonToModel(json, WxInfo.self) as? WxInfo
        }
        return nil
    }
