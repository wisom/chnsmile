...
class LoginViewController: BaseTitleController {
    ...
    override func viewDidLoad() {
        ...
        wxButton.isHidden = WXUtils.shared.isWXApp() ? false : true
    ...
    @IBAction func onWxLoginClick(_ sender: Any) {
...
//        getWxOrigin(phone: username, password: password, userIdentity: self.userIdentity)
        let url="http://121.199.18.201:80/"
        UserDefaults.standard.set(url, forKey: KEY_URL)
        ENDPOINT = url
        
        // 登录
        WXUtils.shared.wxLogin(userIdentity: userIdentity)
    }
...
//        }.disposed(by: disposeBag)
    ...
    func getWxOrigin(phone: String, password: String, userIdentity: Int) {
        Api.shared.origin(phone: phone, isStaff: userIdentity).subscribeOnSuccess { response in
            if let data1 = response?.data {
                print("Origin info:\(data1)")
                var url = ""
