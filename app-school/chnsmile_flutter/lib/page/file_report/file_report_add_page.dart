...
import '../../model/oa_people2.dart';

class FileReportAddPage extends StatefulWidget {
  final Map params;
  String id;
  String formId = "";

  FileReportAddPage({Key key, this.params}) : super(key: key) {
    id = params['id'];
    formId = params['formId'];
  }

  @override
  _FileReportAddPageState createState() => _FileReportAddPageState();
}

class _FileReportAddPageState extends HiState<FileReportAddPage> {
  ...
  List<OAPeople2> approves = []; // 审批人, 通知人
  ...
  handleApproves(List<FileReportApproveInfoList> approves) {
  ...
  loadDefaultData() async {
    try {
      ...
      defaultNotices ??= [];
      if (isEdit) {
        var result = await FileReportDao.detail(widget.id);
        setState(() {
          model = result;
      ...
      setState(() {
        defaultApproves = defaultApproves.isEmpty
            ? (result != null && result.defaultApprove != null)
                ? result.defaultApprove.processMxList
                : defaultApproves
            : defaultApproves;
        ...
        defaultNotices = defaultNotices.isEmpty
            ? (result != null && result.defaultNotice != null)
                ? result.defaultNotice.processMxList
                : defaultNotices
            : defaultNotices;
  ...
  _buildAppBar() {
    ...
    return appBar(isEdit ? '文件发起' : '文件新建', rightTitle: rightTitle,
        rightButtonClick: () {
      if (rightTitle == '删除') {
        delete();
      }
  ...
  delete() async {
    try {
      EasyLoading.show(status: '加载中...');
      var result = await FileReportDao.delete(model.formId);
  ...
  List getApproves() {
    ...
    if (isDefault) {
      for (var i = 0; i < defaultApproves.length; i++) {
        ...
        approves.add(OAPeople2(
            kinds: "1",
            approveId: processInfo.userId,
            floor: processInfo.floor,
            approveName: processInfo.approveName,
            orgName: processInfo.orgName,
            orgId: processInfo.orgId,
            schoolId: processInfo.approveId,
            schoolName: processInfo.approveName));
      }
    ...
    var list = [];
    for (OAPeople2 oaPeople in approves) {
  ...
  onSubmitPressed({isSave = false}) async {
    ...
    try {
      ...
      if (files.isNotEmpty) {
        for (var file in files) {
          if (!file.path.startsWith("http")) {
            ...
            infoEnclosureList.add({
              "id": uploadAttach.id,
              "fileOriginName": uploadAttach.fileOriginName,
              "fileUrl": uploadAttach.fileUrl
            });
          }
        }
      }
      ...
      var result = await FileReportDao.submit(params, isSave: isSave);
              ...
              children: [
                ...
                Positioned(
                    ...
                    child: Container(
                      ...
                      child: Text(processInfo.floor.toString(),
                          style: const TextStyle(
