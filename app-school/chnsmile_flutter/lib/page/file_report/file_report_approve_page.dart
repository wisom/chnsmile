...
class FileReportApprovePage extends StatefulWidget {
  final Map params;
  String id;
  String formId;

  FileReportApprovePage({Key key, this.params}) : super(key: key) {
    id = params['id'];
    formId = params['formId'];
  }

  @override
  _FileReportApprovePageState createState() => _FileReportApprovePageState();
}

class _FileReportApprovePageState extends HiState<FileReportApprovePage> {
  ...
  loadDefaultData() async {
    try {
      ...
      DocumentPcModel result = await FileReportDao.approvePc(widget.formId);
      if (result != null) {
        setState(() {
          model = result;
          defaultApproves ??= [];
  ...
  approve(String content, int status) async {
    try {
      ...
      var result = await FileReportDao.approve(params);
  ...
  addApprove(int index) async {
    try {
      ...
      ListUtils.selecters.forEach((item) async {
        ...
        result = await FileReportDao.addApprove(params);
        if (index == 0) {
          // 审批人
  ...
  removeApprove(String id) async {
    try {
      EasyLoading.show(status: '加载中...');
      var result = await FileReportDao.removeApprove(id);
      EasyLoading.dismiss(animation: false);
      return true;
    } catch (e) {
      EasyLoading.dismiss(animation: false);
