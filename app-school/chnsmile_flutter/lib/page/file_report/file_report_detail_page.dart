...
class FileReportDetailPage extends StatefulWidget {
  final Map params;
  String id;
  ...
  FileReportDetailPage({Key key, this.params}) : super(key: key) {
    id = params['id'];
    type = params['type'];
  ...
  @override
  _FileReportDetailPageState createState() => _FileReportDetailPageState();
}

class _FileReportDetailPageState extends HiState<FileReportDetailPage> {
  ...
  _buildAppBar() {
    ...
    return appBar('文件发起', rightTitle: title, rightButtonClick: () {
      if (title == '删除') {
        delete();
      } else if (title == '撤销') {
        revoke();
  ...
  showApproveDialog() async {
    final result = await BoostNavigator.instance.push('document_approve_page',
        arguments: {"id": model.id, "formId": model.formId});
  ...
  read() async {
    try {
      ...
      var result = await FileReportDao.read(model.id);
  ...
  delete() async {
    try {
      EasyLoading.show(status: '加载中...');
      var result = await FileReportDao.delete(model.id);
  ...
  revoke() async {
    try {
      EasyLoading.show(status: '加载中...');
      var result = await FileReportDao.revoke(widget.id);
      BoostNavigator.instance.pop();
      EasyLoading.dismiss(animation: false);
    } catch (e) {
      EasyLoading.dismiss(animation: false);
  ...
  _buildTop() {
    ...
    return Container(
      ...
      child: Column(
        children: [
          OATwoText(
              '表单编号:', model.formId, '发起日期:', dateYearMothAndDay(model.ddate),
              isLong: true,
              content1Color: Colors.grey,
              content2Color: Colors.grey),
          OATwoText('发起人:', model.cname, '状态:', buildOAStatus(model.status)[1],
              isLong: true,
              content1Color: Colors.grey,
  ...
  _buildAttach() {
    if (model != null && model.attachs != null && model.attachs.isNotEmpty) {
      return Column(
        children: [
  ...
  _buildApply() {
    List<FileReportApproveInfoList> datas = model.approves;
  ...
  _buildApproveTop(FileReportApproveInfoList approve) {
    return Row(
      children: [
        Expanded(
            child: Row(
  ...
  _buildApproveList(FileReportApproveInfoList approve) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
