...
///学生请假-我的通知
class StudentLeaveMyNotifyCard extends StatelessWidget {
  final bool isTeacher;
  final String type;
  final StudentLeave item;
  final ValueChanged<StudentLeave> onCellClick;

  const StudentLeaveMyNotifyCard(
  ...
  _buildTop() {
    String status = isTeacher ? "" : "    " + _getStatusText();
    return Row(
      ...
      children: [
        Row(
          children: [
            Text(
              (item.leaveStudentName ?? "") + "发出的请假申请" + status,
  ...
  _buildBottom() {
    return Row(
      ...
      children: [
        ...
        Text(
          "通知日期 " +
              dateYearMothAndDay((item.dDate ?? "").replaceAll(".000", "")),
