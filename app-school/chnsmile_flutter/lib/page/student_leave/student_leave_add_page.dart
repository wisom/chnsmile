...
import '../../http/dao/student_leave_dao.dart';
...
class StudentLeaveAddPage extends StatefulWidget {
  ...
  StudentLeaveAddPage({Key key, this.params}) : super(key: key) {
    isFromOA = params['isFromOA'];
  }

  @override
  _StudentLeaveAddPageState createState() => _StudentLeaveAddPageState();
}

class _StudentLeaveAddPageState extends HiState<StudentLeaveAddPage> {
  static String TAG = "StudentLeaveAddPage==";

  bool buttonEnable = false;
  int reviewType = 1;
  int hours = 0; //请假小时数
  int leaveType = 0; // 请假类型（1:病假，2:事假）
  String leaveTypeStr = ""; // 请假类型文案
  String leaveName = ""; // 请假名字
  String cname = ""; // 申请人
  String reason = ""; // 请假事由
  String remark = ""; // 备注
  String formId = ""; //
  ...
  _buildAppBar() {
    return appBar('学生请假管理', isEnable: false);
  }
  ...
  showDepartDialog() {
    ...
    print("list:$list");
    showListDialog(context, title: '请选择请假类型', list: list,
        onItemClick: (String type, int index) {
      setState(() {
        //请假类型（1:病假，2:事假）
        leaveTypeStr = sysorgs[index];
        if (sysorgs[index] == "病假") {
          leaveType = 1;
        } else if (sysorgs[index] == "事假") {
          leaveType = 2;
        } else {
          leaveType = 0;
        }
                  ...
                  children: [
                    Row(
                      children: [
                        const Text('    开始时间:',
                            style: TextStyle(
                                fontSize: 13, color: HiColor.color_181717_A50)),
                        Expanded(
                          child: NormalSelect(
                            hint: "请选择开始时间",
                            text: startDate ?? "",
          ...
          children: [
            ...
            Expanded(
              child: NormalInput(
                hint: "请输入单价",
                onChanged: (text) {
                  detailList[option].price = double.parse(text ?? "0");
                },
              ),
            )
          ],
  ...
  _buildBottom() {
    return Container(
      padding: const EdgeInsets.only(left: 10, top: 0, right: 10, bottom: 50),
      color: Colors.white,
      child: Column(
        children: [
          _buildBottomTips(),
  ...
  onSubmitPressed({isSave = false}) async {
    if (leaveType == 0) {
      showWarnToast("请假类型不能为空");
      return;
    }
    if (isEmpty(leaveName)) {
      showWarnToast("请假者不能为空");
      return;
    }
    if (isEmpty(startDate)) {
      showWarnToast("请假开始时间不能为空");
      return;
    }
    if (isEmpty(endDate)) {
      showWarnToast("请假结束时间不能为空");
      return;
    }
    if (hours == 0) {
      showWarnToast("请假小时不能为空");
      return;
    }
    if (isEmpty(reason)) {
      showWarnToast("请假事由不能为空");
      return;
    }
    if (isEmpty(remark)) {
    ...
    try {
      ...
      if (model.fundApplyAccessoryParamList != null &&
      ...
      params['id'] = isSave ? null : formId;
      params['dDate'] = createTime;
      ...
      params['hours'] = hours;
      params['approveParamList'] = getItemParams();

      var result = await StudentLeaveDao.submit(params, isSave: isSave);
  ...
  startTimeClick() {
    Pickers.showDatePicker(
      ...
      onConfirm: (p) {
        String time = '${p.year}-${p.month}-${p.day}  ${p.hour}';
        setState(() {
          startDate = time;
        });
      },
