...
import '../../model/student_leave_detail.dart';
...
class StudentLeaveDetailPage extends StatefulWidget {
  final Map params;
  StudentLeave leave;
  String type; // 1=请假申请、2=请假审批、3=收到通知
  int reviewStatus = 0; // 审核需要用到
  bool isFromOA = false;
  String TAG = "StudentLeaveDetailPage==";

  StudentLeaveDetailPage({Key key, this.params}) : super(key: key) {
    leave = params['item'];
    type = params['type'];
  ...
  @override
  _StudentLeaveDetailPageState createState() => _StudentLeaveDetailPageState();
}

class _StudentLeaveDetailPageState extends HiState<StudentLeaveDetailPage> {
  StudentLeaveDetail model;
  ...
  read() async {
    try {
      ...
      var result = await StudentLeaveDao.read(params);
  ...
  approve(
    ...
    try {
      ...
      params['formId'] = widget.leave.formId;
      var result = await StudentLeaveDao.approve(params);
      var bus = EventNotice();
      bus.formId = widget.leave.formId;
      eventBus.fire(bus);
      PlatformMethod.sentTriggerUnreadToNative();
  ...
  delete() async {
    try {
      EasyLoading.show(status: '加载中...');
      var result = await StudentLeaveDao.delete(widget.leave.id);
      BoostNavigator.instance.pop();
      EasyLoading.dismiss(animation: false);
    } catch (e) {
      EasyLoading.dismiss(animation: false);
  ...
  revoke() async {
    try {
      EasyLoading.show(status: '加载中...');
      var result = await StudentLeaveDao.revoke(widget.leave.id);
      BoostNavigator.instance.pop();
      EasyLoading.dismiss(animation: false);
    } catch (e) {
      EasyLoading.dismiss(animation: false);
  ...
  Future<void> loadData() async {
    try {
      EasyLoading.show(status: '加载中...');
      var result = await StudentLeaveDao.detail(widget.leave.formId ?? "");
      setState(() {
        model = result;
      });
      if (isNotice) {
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          OATwoText('申  请  人:', model.leaveStudentName ?? "", '状态:',
              buildOAStatus(model.status)[1],
              content1Color: Colors.grey,
              // isLong: true,
              content2Color: buildOAStatus(model.status)[0]),
          boxLine(context),
          OAOneText('请假类型:    ', model.leaveType == 1 ? "病假" : "事假"),
          OAOneText(
            '请假者:    ',
            model.leaveStudentName ?? "",
          ),
          OAOneText(
            '请假时间:    ',
            model.dateStart ?? "",
  ...
  _buildApply() {
    ...
    List<StudentLeaveApprove> datas = model?.studentLeaveApproveResultList;
  ...
  _buildApproveTop(StudentLeaveApprove approve) {
    return Row(
      children: [
        Expanded(
            child: Row(
  ...
  _buildApproveList(StudentLeaveApprove approve) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        hiSpace(
