...
class SuperFileDao {
  static list({int pageIndex = 1, int pageSize = 10}) async {
    BaseRequest request = SuperFileListRequest();
    request.add("pageNo", pageIndex).add("pageSize", pageSize);
    var result = await HiNet.getInstance().fire(request);
    return SuperFileModel.fromJson(result['data']);
  }
  ...
  static detail(String formId) async {
    SuperFileDetailRequest request = SuperFileDetailRequest();
    request.add("formId", formId);
    var result = await HiNet.getInstance().fire(request);
    return SuperFileDetailModel.fromJson(result['data']);
  }
