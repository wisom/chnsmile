...
import '../request/clock_in/clock_in_detail_request.dart';
import '../request/clock_in/clock_in_list_request.dart';
...
class ClockInDao {
  ...
  static detail(String punchId) async {
    ClockInDetailRequest request = ClockInDetailRequest();
    request.add("punchId", punchId);
    var result = await HiNet.getInstance().fire(request);
    return ClockInDetail.fromJson(result['data']);
  }

  ///获取打卡列表
  ///isSchool：(0:查询教师的打卡列表，2：查询学生的打卡列表)
  static list(int page, int pageSize, String isSchool) async {
    Map<String, dynamic> params = {};
    params['pageNo'] = page;
    params['pageSize'] = pageSize;
    params['isSchool'] = isSchool;
    ClockInListRequest request = ClockInListRequest();
    request.params = params;
    var result = await HiNet.getInstance().fire(request);
    if (result['code'] == HiConstant.successCode) {
      return ClockInModel.fromJson(result['data']);
    } else {
